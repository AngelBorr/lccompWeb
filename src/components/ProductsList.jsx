...
const ProductList = () => {
    ...
    const handleFilter = () => {
        ...
        if (selectedBrand !== 'todos' && selectedType !== 'todos') {
            filtered = products.filter(prod => prod.prodMarca === selectedBrand && prod.prodType === selectedType);
    ...
    return (
        <>
            <div className="flex justify-center items-center border-[#62a56d] border-[1px] p-2 rounded-lg w-full h-auto gap-2 text-[#54ad99]">        
                <button className="p-2 w-[25%] text-[10px] md:text-sm lg:text-base flex justify-center bg-white border-[1px] border-[#d7f0e9] hover:bg-[#d7f0e9] rounded" id="allProduct" onClick={handleAllProducts}>TODOS</button>
                <select name="marcas" className="p-2 w-[25%] text-[10px] md:text-sm lg:text-base flex justify-center bg-white border-[1px] border-[#d7f0e9] hover:bg-[#d7f0e9] rounded" value={selectedBrand} onChange={handleBrandChange}>  
                ...
                <select name="type" className="p-2 w-[25%] text-[10px] md:text-sm lg:text-base flex justify-center bg-white border-[1px] border-[#d7f0e9] hover:bg-[#d7f0e9] rounded" value={selectedType} onChange={handleTypeChange}>  
                ...
                id="downloadsList" href='/downloads/LISTA LC.pdf' download={'LISTA LC.pdf'} target='_blank'>DESCARGAR LISTADO</a>
            </div>
                ...
                {filteredProducts.map((prod) => (
                    <div key={prod.id} className="w-full h-full p-3 border rounded-xl bg-gradient-to-r from-[#62a56d] to-[#bdddbf]">
                        <div className="w-full h-[270px] aspect-square p-4 bg-[#ddeedd] rounded-lg">
                            <img className="object-scale-down w-full h-full rounded-md aspect-square " src={prod.prodImg1 || '/img/camara-fotografica.png'} alt={prod.prodName} loading='lazy'/>
                        </div>
                        <div className="p-2 border-t-2 border-[#62a56d]">
                            ...
                            <p className="flex justify-center text-base font-extrabold text-[#41884f] items-center">Valor = {prod.prodPrecio}</p>
                        </div>
                        <div className="flex justify-center w-full items-center">
                            <a className="border border-[#41884f] rounded-lg text-[#41884f] w-24 h-8 flex justify-center items-center hover:bg-[#265632] hover:text-white" href={`/${prod.id}.productDetall`}>Más Info</a>
                        </div>
                    </div>
                ))}
            </main>
